...
# They are debugged
{
  ...
  # Checks if a given directory already exists on the system. 
  # If it does not exist, it creates it first and then defines it as a working directory.
  ...
  # Length of a vector, with the possibility to compute missing values or not.
  # NNa(c(2,3,NA, 4)); NNa(c(2,3,NA, 4),TRUE)
...
# They are debugged
{
  ...
  # Generates Pos Hoc paired contrasts based on the number of levels of the variable.
  # GenMultCon(3)
  ...
  # Generates the different combinations involved in the Pos Hoc analysis.
  # mkIdxJ(c("A","B","C"))
  # mkIdxJ(c(1:3))
  mkIdxJ<-function(nv) {
    nLev=length(nv)
  ...
  SummUja <-function(DatPas,DPrec=3,Recor=.20) {
    ...
    list("Basicos"=Basicos,"Robustos"=Robustos,"Forma"=Forma,"Especiales"=Especiales,"Cualitativas"=Cualitativas)
  }
  
  # rndBlock(c("Exp","Control"),10)  
  ...
  Com2Seq<-function(xP,NmSuj="Experiment",NmDepV="Dig"){
    x<- copy(xP) %>%
      dplyr::rename(Sj=eval(NmSuj)) %>%
      dplyr::rename(DV=eval(NmDepV)) %>%
      dplyr::select(.,c(Sj,DV)) %>%
      data.table()
    LosSubj<-unique(x$Sj)
    Lev0 = levels(x$DV)
    NumLev0=length(Lev0)
    x <- x %>% mutate(DV = factor(DV, labels = c(1:NumLev0)))
    ExtracM<-matrix(0,nrow = NumLev0,ncol=NumLev0);
    for (sj in 1:length(LosSubj)) {
      Subjs<-x[Sj==LosSubj[sj]]$DV
      for (i in 1:(length(Subjs)-1)) {
        ExtracM[Subjs[i],Subjs[i+1]]<-ExtracM[Subjs[i],Subjs[i+1]]+1
      }
    }
    rownames(ExtracM)<-colnames(ExtracM)<-Lev0
    ExtracM
  }
...
# Visualization of a correlation matrix using ggplot2
{
  ...
  ggcorrplot.vMM <- function(corr,
    ...
    corr <- corr %>% mutate(Var2=factor(Var2,levels = rev(levels(Var1))))
    # No va bien: 
    # levels(corr$Var2)<-rev(levels(corr$Var1))
    ...
    # p <- p + ggplot2::scale_fill_gradient(low = "blue", high = "red",midpoint = 0.5,name = legend.title, limit = c(0, 1))
  ...
  CorPlotMM.23 <-function(DatP,PathP,sil=FALSE,titp="", MaxSc=1) {
    colorMM=colorRampPalette(c("cyan","blue"))(200)
    if (!sil) corrplot::corrplot(DatP/sum(DatP),title = paste0("\n\n",titp),tl.srt = 30,is.corr = F,col.lim = c(0,MaxSc), 
                                 win.asp = 1, method = "circle",tl.col="black",col= colorMM, type = "full")  -> p
    tDatP=t(DatP)
    ggCor = ggcorrplot.vMM(tDatP/sum(tDatP), method = "circle",legend.title="Rate") 
    
    if (MaxSc!=1) ggCor <- ggCor +
      scale_fill_gradient2(limit = c(0,MaxSc), low = "blue", high =  "red", mid = "white", midpoint = MaxSc/2)
    
    pdf(paste0(PathP,".pdf"))
    corrplot::corrplot(DatP/sum(DatP),tl.srt = 30,is.corr = F,col.lim = c(0,MaxSc), 
                       win.asp = 1, method = "circle",tl.col="black",col= colorMM, type = "full")
    plot(ggCor)
    dev.off()
    write.csv2(x = DatP,file = paste0(PathP,"_DigPatterns.csv"))
    
    ResCh=chisq.test(c(DatP))
    ResCh2= with(ResCh, 
                 paste0("N = ",sum(DatP),", [",paste(DatP,collapse = ","),"], ",
                        "X2(",round(parameter[[1]],2),") = ",
                        frmMM(statistic[[1]],2),", p =",frmMM(p.value,4)))
    ResF <- list(
      ChiTest = ResCh2,
      GrpPat = ggCor)
    ResF
